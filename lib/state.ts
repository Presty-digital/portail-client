...
export function normalizeInstitut(i:any={}){return {...i,id:i.id||uid(),name:i.name||i.companyName||"Compte client",companyName:i.companyName||i.name||"Compte client",categories:arr(i.categories).length?arr(i.categories):DEFAULT_CATEGORIES,pipeline:arr(i.pipeline).length?arr(i.pipeline):DEFAULT_PIPELINE,active:i.active!==false};}
