...
async function ingest(req,forcedInstitutId=""){
 ...
 const incomingCategory=String(latest?.category||mapping.category||existing?.latestCategory||existing?.category||"Autres");
 ...
 const categories=[...new Set([...previousCategories,...submissionCategories,incomingCategory].filter(Boolean))];
 const firstStatus=targetClient?.pipelineConfig?.[0]?.label||targetClient?.pipeline?.[0]||"Nouveau";
 const commercialByCategory={...(existing?.commercialByCategory||{})};
 if(existing?.category&&!commercialByCategory[existing.category])commercialByCategory[existing.category]={category:existing.category,status:existing.statut||firstStatus,value:Number(existing.value||0),converted:Boolean(existing.converted),updatedAt:existing.updatedAt||nowIso()};
 for(const cat of categories){if(!commercialByCategory[cat])commercialByCategory[cat]={category:cat,status:firstStatus,value:0,converted:false,updatedAt:nowIso()}}
 const activeTracking=commercialByCategory[incomingCategory]||{status:firstStatus,value:0,converted:false};
 const patch=normalizeLead({...existing,id:existing?.id||uid(),institutId,dateContact:existing?.dateContact||today(),prenom:pick(b,"first_name","firstName","prenom","contact.firstName")||existing?.prenom||"",nom:pick(b,"last_name","lastName","nom","contact.lastName")||existing?.nom||"",telephone:phone||existing?.telephone||"",email:email||existing?.email||"",categories,latestCategory:incomingCategory,commercialByCategory,category:incomingCategory,typeSoin:latest?.service||mapping.service||existing?.typeSoin||"",formName,source:formName,channel:existing?.channel||"GoHighLevel",problematique:existing?.problematique||"",additionalInfo:{...(existing?.additionalInfo||{}),...extraInfo(b)},formSubmissions,statut:activeTracking.status||firstStatus,converted:Boolean(activeTracking.converted),value:Number(activeTracking.value||0),ghlContactId:contactId||existing?.ghlContactId||"",ghlLocationId:locationId||existing?.ghlLocationId||"",ghlFormId:String(latest?.formId||explicitFormId||existing?.ghlFormId||""),updatedAt:nowIso(),activity});
