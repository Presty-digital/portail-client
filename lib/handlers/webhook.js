...
function plausibleQuestionLabel(value,id=''){
...
function extractPublicFormLabels(html,wantedIds=[]){
 ...
 for(const rawId of wantedIds){
  ...
  while(guard++<12){
   ...
   const left=Math.max(0,pos-1800),right=Math.min(source.length,pos+id.length+1800),chunk=source.slice(left,right),idPos=pos-left;
   const candidates=[];
   const jsonRe=new RegExp('(?:"|\\\\")('+keys+')(?:"|\\\\")\\s*:\\s*(?:"|\\\\")([^"\\n\\r]{2,260}?)(?:"|\\\\")','gi');
   let m;while((m=jsonRe.exec(chunk))){const label=plausibleQuestionLabel(m[2],id);if(label)candidates.push({label,key:m[1],distance:Math.abs(m.index-idPos)})}
   const attrRe=/(?:data-label|aria-label|placeholder|title)\s*=\s*["']([^"']{2,260})["']/gi;
   while((m=attrRe.exec(chunk))){const label=plausibleQuestionLabel(m[1],id);if(label)candidates.push({label,key:'label',distance:Math.abs(m.index-idPos)})}
   const labelRe=/<label\b[^>]*>([\s\S]{1,400}?)<\/label>/gi;
   while((m=labelRe.exec(chunk))){const label=plausibleQuestionLabel(m[1],id);if(label)candidates.push({label,key:'label',distance:Math.abs(m.index-idPos)})}
   for(const c of candidates){
    const rank=/^(label|question|questionLabel|fieldLabel|customFieldLabel)$/i.test(c.key)?0:/^(title|placeholder)$/i.test(c.key)?1:2;
    const score=rank*10000+c.distance;
...
async function fetchPublicFormFieldLabels(formId,wantedIds=[]){
 ...
 try{
  ...
  const html=await r.text();return extractPublicFormLabels(html,wantedIds)
...
async function ingest(req,forcedInstitutId=""){
 ...
 const missingIds=submissionMachineIds(fetched).filter(id=>!customFieldMap.has(id)&&!customFieldMap.has(normKey(id)));
 if(missingIds.length){
  const formIds=[...new Set(fetched.map(x=>String(x?.formId||'')).filter(Boolean))];
  for(const fid of formIds){const m=await fetchPublicFormFieldLabels(fid,missingIds);for(const [k,v] of m)if(!customFieldMap.has(k)){customFieldMap.set(k,v);publicFormMap.set(k,v)}}
 }
