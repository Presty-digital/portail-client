...
async function fetchLocationCustomFields(state,locationId){
 ...
 let token=await getValidLocationToken(state,locationId,saveState);
 let {r,b}=await request(token);
 // Quand un nouveau scope est ajouté à la version HighLevel, un ancien token Location
 // peut continuer à porter les anciens scopes. On génère alors automatiquement un
 // nouveau token Location à partir du token Company, sans action utilisateur.
 if((r.status===401||r.status===403)&&state?.ghlOAuth?.agencyToken){
  token=await saveFreshLocationToken(state,locationId);
  ({r,b}=await request(token));
 }
 if(!r.ok)throw new Error(Array.isArray(b?.message)?b.message.join(', '):b?.message||b?.error||`Erreur GHL custom fields (${r.status})`);
 const map=new Map();
 for(const f of (Array.isArray(b?.customFields)?b.customFields:[])){
  const label=String(f?.name||f?.placeholder||f?.fieldKey||'Champ personnalisé').trim();
  if(f?.id){map.set(String(f.id),label);map.set(normKey(String(f.id)),label)}
  if(f?.fieldKey){map.set(String(f.fieldKey),label);map.set(normKey(String(f.fieldKey)),label);const short=String(f.fieldKey).replace(/^contact\./i,'');map.set(short,label);map.set(normKey(short),label)}
 }
 return map
}
...
async function ingest(req,forcedInstitutId=""){
 ...
 return NextResponse.json({ok:true,leadId:patch.id,created:!existing,institutId,submissions:formSubmissions.length,newSubmissions:Math.max(0,formSubmissions.length-previousSubs.length),submissionError:submissionError||undefined,customFieldError:customFieldError||undefined});
}
