...
export async function POST(request) {
  try {
    ...
    const user = { id: uid(), authUserId: authUser.id, name, email, passwordHash: hashPassword(password), role: "agency_admin", institutId: null, accessIds: [], active: true, createdAt: new Date().toISOString() };
    ...
    response.cookies.set(cookieName(), signSession({ userId: user.id, role: user.role, institutId: null, accessIds: [], exp: Date.now() + 604800000 }), { httpOnly: true, secure: process.env.NODE_ENV === "production", sameSite: "lax", path: "/", maxAge: 604800 });
    return response;
  } catch (error) {
