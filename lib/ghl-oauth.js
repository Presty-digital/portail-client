...
export async function getInstalledLocations(state,saveState){
  // V21.32 PRODUCTION-SAFE : la découverte des installations ne consomme JAMAIS
  // le refresh token. Un clic sur « Rafraîchir » est une lecture GHL, point.
  // Cela supprime définitivement la boucle 401 -> rotation -> refresh token invalid.
  let agency=currentAgencyToken(state);
  try{return await fetchInstalledLocations(agency)}catch(e){
    if(e?.status===401){
      // Une autre instance Vercel peut avoir enregistré un token plus récent : relire
      // Supabase et retenter uniquement si l'access token a réellement changé.
      try{
        const {loadState}=await import("@/lib/db");
        const latest=await loadState();
        const newer=latest?.ghlOAuth?.agencyToken;
        if(newer?.accessToken&&newer.accessToken!==agency.accessToken){
          state.ghlOAuth=latest.ghlOAuth;
          return await fetchInstalledLocations(newer);
        }
      }catch(retry){if(retry?.status!==401)throw retry}
      const scopes=[...scopeSet(agency)];
      throw new Error(`HighLevel refuse le token agence courant (401) sur /oauth/installed-locations. userType=${agency.userType||"inconnu"}; oauth.readonly=${scopes.includes("oauth.readonly")?"oui":"non"}; companyId=${agency.companyId?"présent":"absent"}. Aucun refresh token n’a été consommé. Détail HighLevel : ${e.message||"(vide)"}`);
    }
    if(e?.status===403){
      const scopes=[...scopeSet(agency)];
      throw new Error(`HighLevel interdit /oauth/installed-locations (403). oauth.readonly=${scopes.includes("oauth.readonly")?"oui":"non"}. Aucun token n’a été renouvelé. Détail HighLevel : ${e.message||"(vide)"}`);
    }
    throw e;
  }
}
