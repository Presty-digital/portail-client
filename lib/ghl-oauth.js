...
export async function getValidAgencyToken(state,saveState){
  ...
  if(!item.refreshToken)throw new Error("Le token agence GHL a expiré et aucun refresh token n’est disponible. Reconnectez GoHighLevel.");
  try{
    item=normalizeToken(await refreshAccessToken(item.refreshToken,"Company"),item);
  }catch(error){
    // Les refresh tokens HighLevel sont à usage unique. Si une autre requête a déjà
    // effectué la rotation, relire le token fraîchement persisté avant de conclure
    // que l'autorisation agence est cassée.
    if(isInvalidRefreshTokenError(error)){
      try{
        const {loadState}=await import("@/lib/db");
        const fresh=await loadState();
        const latest=fresh?.ghlOAuth?.agencyToken;
        if(latest?.accessToken&&latest?.refreshToken&&latest.refreshToken!==item.refreshToken){
          state.ghlOAuth=fresh.ghlOAuth;
          return latest;
        }
      }catch{}
      throw new Error("Le refresh token agence HighLevel a déjà été utilisé ou invalidé. Cliquez une seule fois sur « Reconnecter GoHighLevel », puis sur « Rafraîchir les sous-comptes ».");
    }
    throw error;
  }
...
export async function getInstalledLocations(state,saveState){
  ...
  try{
    // IMPORTANT : approvedLocations n'est PAS la liste des installations Marketplace.
    // HighLevel peut y renvoyer tous les sous-comptes approuvés par le token Company
    // (ex. 52 comptes agence) même si Presty CRM n'est installé que sur 23 comptes.
    // La seule source de vérité pour l'écran "Sous-comptes disponibles" est
    // GET /oauth/installed-locations?isInstalled=true.
    return await fetchInstalledLocations(agency);
  }catch(e){
    // Ne jamais retomber sur agency.approvedLocations : ce fallback créait des
    // faux sous-comptes "disponibles" dans Presty et expliquait le compteur 52.
