...
async function fetchInstalledLocations(agency){
  ...
  // V21.23 — HighLevel OAuth v3 pagine /oauth/installed-locations.
  // pageSize vaut 20 par défaut : sans pagination, Presty ne voyait donc que
  // les 20 premières installations même si davantage de sous-comptes étaient installés.
  // On demande le maximum autorisé (100) et on suit nextPageToken jusqu'à la fin.
  const locations=[];
  let pageToken="";
  ...
  for(let page=0;page<100;page+=1){
    ...
    const r=await fetch(`${API_BASE}/oauth/installed-locations?${q}`,{
      headers:{Accept:"application/json",Authorization:`Bearer ${agency.accessToken}`,Version:API_VERSION},
      cache:"no-store"
    });
    ...
    const list=b?.items||b?.locations||b?.data?.items||b?.data?.locations||(Array.isArray(b?.data)?b.data:null)||b?.results||[];
    if(Array.isArray(list))locations.push(...list);
    const pagination=b?.pagination||b?.data?.pagination||{};
    const next=String(pagination?.nextPageToken||pagination?.next_page_token||"");
    const hasNext=pagination?.hasNextPage??pagination?.has_next_page??Boolean(next);
    if(!hasNext||!next||seenTokens.has(next))break;
    seenTokens.add(next);
    pageToken=next;
  }
  // Dédupliquer par locationId au cas où une page GHL contiendrait un doublon.
  const unique=new Map();
  for(const loc of locations){
    const id=String(loc?.locationId||loc?._id||loc?.id||"");
...
export async function syncInstalledLocations(state,saveState){
  ...
  for(const loc of locations){
    ...
    // Un nouveau consentement OAuth peut ajouter des scopes sans invalider les anciens
    // tokens Location stockés. Régénérer le token si son scope est incomplet.
    // Le token Company ne reflète pas toujours les scopes Sub-Account dans son champ `scope`.
    // On ne s'en sert donc plus pour décider si un Location Token doit être renouvelé.
    // Si le Location Token stocké ne contient pas le scope Custom Fields attendu par Presty,
    // on le régénère directement depuis le Company Token.
    const locationNeedsUpgrade=!hasScopes(item,["locations/customFields.readonly"]);
    const locationExpired=Boolean(item?.expiresAt&&new Date(item.expiresAt).getTime()<=Date.now()+60000);
    // V21.22 : lors d’un rafraîchissement admin, ne jamais tenter de conserver un
    // Location Token expiré. On en génère directement un nouveau depuis le Company
    // Token, ce qui remplace aussi un éventuel refresh token Location devenu invalide.
    if(!item?.accessToken||locationNeedsUpgrade||locationExpired)item=await exchangeLocationToken(state,locationId,saveState);
...
export async function getValidLocationToken(state,locationId,saveState,requiredScopes=[]){
  ...
  // Si le token Location est absent OU a été créé avant l'ajout d'un scope,
  // le recréer depuis le token Company actuellement autorisé.
  if((!item?.accessToken||!hasScopes(item,requiredScopes))&&state?.ghlOAuth?.agencyToken){
    // Ne pas bloquer sur les scopes du Company Token : HighLevel peut ne pas y exposer
    // les scopes Sub-Account. Le Location Token généré est la source de vérité.
    await getValidAgencyToken(state,saveState);
  ...
  if(requiredScopes.length&&!hasScopes(item,requiredScopes)){
    throw new Error(`Le token du sous-compte GHL ne contient pas le scope requis : ${requiredScopes.join(", ")}`);
  }
  ...
  let refreshed;
  try{
    refreshed=normalizeToken(await refreshAccessToken(item.refreshToken,"Location"),item);
  }catch(error){
    // V21.22 : certains sous-comptes conservent un refresh token Location que GHL
    // invalide après une réautorisation/rotation. Si l’accès agence Company est
    // toujours valide, on recrée immédiatement le Location Token au lieu de renvoyer
    // « This refresh token is invalid » au client.
    if(isInvalidRefreshTokenError(error)&&state?.ghlOAuth?.agencyToken){
      item=await exchangeLocationToken(state,locationId,saveState);
      state.ghlOAuth={...(state.ghlOAuth||{}),installations:installs.map(x=>String(x.locationId)===String(locationId)?item:x),lastError:"",updatedAt:new Date().toISOString()};
      await saveState(state);
      return item.accessToken;
    }
    throw error;
  }
  // Un refresh conserve les scopes d'origine : si un scope est encore manquant,
  // forcer un nouveau Location token depuis le Company token.
  if(requiredScopes.length&&!hasScopes(refreshed,requiredScopes)&&state?.ghlOAuth?.agencyToken){
    item=await exchangeLocationToken(state,locationId,saveState);
  }else item=refreshed;
  state.ghlOAuth={...(state.ghlOAuth||{}),installations:installs.map(x=>String(x.locationId)===String(locationId)?item:x)};
  await saveState(state);return item.accessToken;
}
