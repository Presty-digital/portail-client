...
export async function exchangeAuthorizationCode(code,redirectUri){
  ...
  return tokenRequest({client_id:clientId,client_secret:clientSecret,grant_type:"authorization_code",code:String(code||""),user_type:"Company",redirect_uri:String(redirectUri||REDIRECT_URI)});
}
...
// V21.44 — Persistance d'un Location Token sans jamais réécrire une copie périmée d'app_state.
// On relit l'état le plus récent, on ne modifie QUE ghlOAuth.installations pour ce sous-compte,
// puis écriture conditionnelle (CAS sur updated_at) avec relecture en cas de conflit. La copie en mémoire de l'appelant est mise à jour
// (ghlOAuth uniquement) pour les appels suivants de la même requête, mais n'est jamais sauvegardée ici.
export async function persistLocationToken(state,locationId,item,saveState){
  const merge=list=>{const arr=Array.isArray(list)?list:[];return arr.some(x=>String(x.locationId)===String(locationId))?arr.map(x=>String(x.locationId)===String(locationId)?{...x,...item}:x):[...arr,item]};
  const stamp=new Date().toISOString();
  try{
    const {loadStateVersioned,saveStateIfUnchanged}=await import("@/lib/db");
    let saved=false;
    // Écriture conditionnelle (CAS) : en cas de modification concurrente d'app_state,
    // on relit et on réapplique uniquement le changement de ghlOAuth.
    for(let attempt=1;attempt<=5&&!saved;attempt++){
      const {state:target,previous,updatedAt}=await loadStateVersioned();
      target.ghlOAuth={...(target.ghlOAuth||{}),installations:merge(target.ghlOAuth?.installations),lastError:"",updatedAt:stamp};
      saved=await saveStateIfUnchanged(target,updatedAt,previous);
      if(!saved)await new Promise(r=>setTimeout(r,80*attempt+Math.floor(Math.random()*120)));
    }
    if(!saved)console.error("[ghl-oauth] Location Token non persisté après 5 conflits (sera régénéré au prochain appel)");
  }catch(error){
    // Échec de persistance : le token reste utilisable pour cette requête ; il sera
    // simplement régénéré au prochain appel. On n'écrit surtout pas la copie périmée.
    console.error("[ghl-oauth] persistance du Location Token impossible",error?.message||error);
  }
  if(state&&typeof state==="object")state.ghlOAuth={...(state.ghlOAuth||{}),installations:merge(state.ghlOAuth?.installations),lastError:"",updatedAt:stamp};
  return item;
}
...
export async function getValidLocationToken(state,locationId,saveState,requiredScopes=[]){
  ...
  if((!item?.accessToken||expired||!hasScopes(item,requiredScopes))&&state?.ghlOAuth?.agencyToken){
    ...
    // V21.44 : ne réécrit plus la copie `state` de l'appelant (souvent périmée : webhook,
    // formulaires...). Le token est persisté depuis un état frais, ghlOAuth uniquement.
    await persistLocationToken(state,locationId,item,saveState);
  }
