...
const INSTALLED_LOCATIONS_VERSION="2021-07-28";
...
async function fetchInstalledLocations(agency){
  ...
  // V21.30 — retour à l'endpoint OAuth historique/stable utilisé par HighLevel
  // pour les apps Marketplace. Il accepte jusqu'à 100 résultats par appel.
  // Cela évite la migration v3 qui a introduit le blocage actuel tout en supprimant
  // la limite implicite de 20 comptes.
  const locations=[];
  let skip=0;
  for(let page=0;page<100;page+=1){
    ...
    const versionId=process.env.GHL_VERSION_ID||process.env.GHL_APP_VERSION_ID||process.env.GHL_APP_ID||"";
    if(versionId)q.set("versionId",String(versionId));
    const r=await fetch(`${API_BASE}/oauth/installedLocations?${q}`,{
      headers:{Accept:"application/json",Authorization:`Bearer ${agency.accessToken}`,Version:INSTALLED_LOCATIONS_VERSION},
      cache:"no-store"
    });
    ...
    if(list.length<100 || (total>0 && locations.length>=total))break;
    skip+=list.length;
