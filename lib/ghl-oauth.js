...
export async function getInstalledLocations(state,saveState){
  ...
  // V21.29 — les permissions cochées dans Marketplace ne suffisent pas :
  // elles doivent aussi être présentes dans LE TOKEN délivré. Une ancienne
  // variable GHL_SCOPES Vercel pouvait produire un Company token sans
  // oauth.readonly malgré la permission activée dans l'app.
  if(!hasScopes(agency,["oauth.readonly"])) {
    throw new Error(`Le token agence actuellement délivré par HighLevel ne contient pas oauth.readonly (scopes reçus : ${agency.scope||"aucun"}). Reconnectez GoHighLevel une seule fois avec la V21.29.`);
  }
