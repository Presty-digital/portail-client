...
export async function syncInstalledLocations(state,saveState){
  ...
  for(const loc of locations){
    ...
    const agency=state?.ghlOAuth?.agencyToken;
    const agencyHasCustomFields=hasScopes(agency,["locations/customFields.readonly"]);
    const locationNeedsUpgrade=agencyHasCustomFields&&!hasScopes(item,["locations/customFields.readonly"]);
...
export async function getValidLocationToken(state,locationId,saveState,requiredScopes=[]){
  ...
  if((!item?.accessToken||!hasScopes(item,requiredScopes))&&state?.ghlOAuth?.agencyToken){
    const agency=await getValidAgencyToken(state,saveState);
    if(requiredScopes.length&&!hasScopes(agency,requiredScopes)){
      throw new Error(`Le token agence GHL ne contient pas encore le scope requis : ${requiredScopes.join(", ")}. Réautorisez la version HighLevel qui contient ce scope.`);
    }
