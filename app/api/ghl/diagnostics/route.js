...
export async function GET(req){
 try{
  ...
  const state=await loadState();const url=new URL(req.url);const id=url.searchParams.get('id')||'',locationId=url.searchParams.get('locationId')||'';
  const all=Array.isArray(state.ghlDiagnostics)?state.ghlDiagnostics:[];
  const diagnostics=id?all.filter(x=>String(x.id)===id):locationId?all.filter(x=>String(x.locationId)===locationId):all;
